...
header-includes:
   ...
   - \usepackage{amsmath}
   - \usepackage{float}
   - \usepackage[hyperindex,breaklinks,colorlinks, urlcolor=blue]{hyperref}
---
...
## 4. Understanding the dynamics of financial returns: non-constant volatility models
...
To elucidate the magnitude of differences between the two fitted GJR-GARCH(1,1) models, Figure \ref{fig:GJR_AIC}(c) depicts a delta in the AIC scores. From this, it can be concluded that when the model assuming normality had the lowest AIC score (indicating the best fit), the differential in AIC scores remained minimal. However, as denoted by the green area on the chart, the GJR-GARCH(1,1) model with a t-Student assumption consistently exhibited a marked difference. This underlines the model's superior fit performance. 

Earlier, we spotlighted the persistent nature of the leverage effect over a subsequent five-day period (Figure \ref{fig:leverageEffect}). This finding is crucial as, notwithstanding the GJR-GARCH(1,1) presents inadequacy in accurately modelling the lower tail of the standardized residuals, it displayed a superior fit performance compared to a conventional GARCH(1,1) model. This attribute underscores the relevance of the GJR-GARCH(1,1) model, as evidenced by its lower AIC score described before. The persistent influence of the leverage effect over a five-day period appears to be a key factor in this improved fit, thus favoring the GJR-GARCH(1,1) model over its conventional counterpart.
...
\leftskip=2cm
1. If the observed deficit ratio exceeds $p$, the second alternative hypothesis posits that the true daily probability of an exception is greater than $p$.

2. Conversely, if the deficit ratio is less than $p$, the second alternative hypothesis contends that the true daily probability of an exception is beneath $p$.

\leftskip=0cm
Given these two potential alternative hypotheses, the decision to accept or reject the model is contingent upon both the observed deficit ratio and a confidence level ($\alpha=95\%$). If the ratio exceeds $p$, equation \ref{eq:19} is employed to calculate the likelihood that the VaR level will be surpassed on $m$ or more days. On the other hand, when the deficit ratio is below $p$, equation \ref{eq:20} is used to determine the probability of the VaR level being exceeded on $m$ or fewer days. In both scenarios, the model performance is accepted if the outcome exceeds the significance level of five percent (which is $1-\alpha$).
\
\begin{table}[H]
...
In the realm of Value-At-Risk (VaR) forecasting, precision is imperative. Our exploration of volatility models emphasized this foundational principle. We began with basic models grounded on static assumptions and quickly discovered the necessity for more adaptable tools given the ever-evolving market dynamics.

The assumption that the daily returns of the S&P500 Index adhered to a normal distribution was swiftly debunked. Distinctly fatter tails in the distribution showcased the limitation of basic volatility models in accounting for extreme tail events, thus impairing the reliability of VaR predictions. The data indicated time-variant volatility (clustering), contradicting assumptions of constant volatility in the most basic models. This led to our exploration of non-constant volatility models, from the basic EWMA and GARCH(1,1) to more sophisticated ones like GJR-GARCH(1,1) and EGARCH(1,1). These advanced models recognize the asymmetry in the relationship between volatility and returns - the leverage effect. Employing the Akaike Information Criterion (AIC), we found that market conditions often dictate the superiority of either GJR-GARCH or EGARCH volatility models. To further enhance VaR forecasting accuracy, we combined both models based on an AIC threshold, which, upon back-testing, proved to be the most effective.

Our path was interspersed with hurdles. The meticulous task of refining the EGARCH model, particularly employing a grid-search method to identify the ideal t-Student distribution shape for the standardized residuals, proved to be demanding and often did not converge satisfactorily. However, comparing the histogram of the standardized residuals with a standard normal distribution suggested that adopting a standard normal distribution as a simplifying assumption seems justifiable.

Beyond the realm of academia, our work has broader implications. Past financial crisis reveal the consequences of imprecise risk predictions. Accurate VaR models act as defensive barriers against such large-scale disruptions, emphasizing their role in the stability of global economies.

However, it is crucial to recognize that while our study has enhanced VaR predictions, the world of risk management continually evolves. While VaR offers a glimpse into the risk landscape, tools like the Expected Shortfall (ES) provide deeper insights into potential extreme deficits.

To conclude, this dissertation underscores the balance between precision and adaptability in VaR forecasting. It serves as a reminder of the ongoing journey to refine understanding, the need for incessant adaptation, and the crucial role of resilient risk models in stabilizing financial frameworks. In the vast ocean of global finance, may our models be as fluid and responsive as the markets, ensuring we stay a step ahead in protecting our investments and the broader economic ecosystem.
\clearpage
...
\textbf{Access to Codes and Files:} All computational resources, scripts, and ancillary files related to this dissertation are publicly accessible for transparency and reproducibility purposes. The interested reader can access and review them at the following repository: \href{https://github.com/CValdiviaBarros/my-dissertation-codes.git}{https://github.com/CValdiviaBarros/my-dissertation-codes.git}. The repository provides a structured collection of all the codes and data files used in the various chapters and sections of this research work. I encourage those interested in a deeper understanding or replication of the presented work to visit the repository and examine the materials firsthand.
